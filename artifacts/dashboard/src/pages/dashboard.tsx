import { useGetAccount, useGetPositions, useGetSignals, useGetBotStatus, useClosePosition, useGetPerformance, useGetSettings, useUpdateSettings, useEmergencyStop } from "@workspace/api-client-react";
...
import { XCircle, Activity, DollarSign, PieChart, ChevronRight, Pencil, Check, X, ShieldAlert, Target, RotateCcw, AlertOctagon } from "lucide-react";
    ...
    emergencyStop.mutate(undefined, {
      onSuccess: (data) => {
        ...
        const msg = data.closedPositions > 0
          ? `Closed ${data.closedPositions} position${data.closedPositions !== 1 ? "s" : ""}. Bot is now offline.`
          : "Bot is offline. No open positions found.";
        toast({ title: "EMERGENCY STOP EXECUTED", description: msg });
      },
      onError: () => {
        toast({ title: "Emergency stop failed — check logs", variant: "destructive" });
      },
    });
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        ...
        {/* ── EMERGENCY STOP — Desktop Dashboard (hidden on mobile, shown in bottom nav there) ── */}
          ...
          <AlertDialog>
            <AlertDialogTrigger asChild>
              <Button
                variant="destructive"
                size="sm"
                className="font-mono text-xs font-bold h-9 px-4 ring-1 ring-destructive/50 shadow-[0_0_16px_rgba(239,68,68,0.3)] hover:shadow-[0_0_24px_rgba(239,68,68,0.5)] transition-shadow"
            ...
            <AlertDialogContent className="border-destructive/50 bg-card">
              <AlertDialogHeader>
                ...
                <AlertDialogDescription className="font-mono text-xs leading-relaxed">
                  This will instantly close <strong className="text-foreground">ALL open positions</strong> on
                  Capital.com and take the bot offline. This action cannot be undone.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                    ...
                    {positions.map((pos) => (
                      <TableRow key={pos.dealId}>
                        ...
                        <TableCell className="hidden sm:table-cell">
                          {formatNumber(pos.openLevel ?? undefined)}
                        </TableCell>
