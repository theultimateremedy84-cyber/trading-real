...
import { CAPITAL_DEMO_URL as DEMO_URL, CAPITAL_LIVE_URL as LIVE_URL, normalizeCapitalUrl } from "../lib/capitalUrl";
...
function serializeSettings(s: typeof botSettingsTable.$inferSelect) {
  return {
    ...
    // SINGLE-MARKET MODE: BTCUSD is the only market the bot scans/trades.
    enabledMarkets: ["BTCUSD"],
...
router.put("/settings", async (req, res) => {
  try {
    ...
    // SINGLE-MARKET MODE: BTCUSD is the only market the bot may trade, so any
    // incoming market list is normalised to BTCUSD.
    if (body.enabledMarkets !== undefined) updateData.enabledMarkets = "BTCUSD";
    ...
    if (body.capitalApiUrl !== undefined) {
      updateData.capitalApiUrl = normalizeCapitalUrl(
        body.capitalApiUrl,
        body.isDemo ?? true,
      );
    }
